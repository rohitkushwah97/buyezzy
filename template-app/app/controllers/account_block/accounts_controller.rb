module AccountBlock
  class AccountsController < AccountBlock::ApplicationController
    ...
    skip_before_action :validate_json_web_token, :authenticate_account, only: [:resend_email]
    before_action :validate_json_web_token, :authenticate_account, only: [:search, :change_email_address, :change_phone_number, :logged_user, :delete_user]
    ...
    def resend_email
      ...
      if account.present?
        if account.activated?
          return render json: { message: "Email is already verified." }, status: 422
        else
          AccountActivationMailer.with(account: account).activation_email.deliver_now
          return render json: { message: "Email sent successfully." }, status: 200
        end
      else
        return render json: { error: "Email does not exist." }, status: 404
      end
    end
    ...
    def change_email_address
      ...
      if account
        return render json: {errors: "Entered email already exist with account please add another email"}, status: :unprocessable_entity
    ...
    def change_phone_number
      ...
      if @account.update(full_phone_number: params["full_phone_number"])
        render json: AccountSerializer.new(@account).serializable_hash, status: :ok
      elsif @account.errors.messages.present?
    ...
    def details
      @account = AccountBlock::Account.find(params[:id])  # Fetch the user by ID
    end
