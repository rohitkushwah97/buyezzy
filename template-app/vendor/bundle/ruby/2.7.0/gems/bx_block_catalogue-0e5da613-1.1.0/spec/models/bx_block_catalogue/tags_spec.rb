...
RSpec.describe BxBlockCatalogue::Tag, type: :model do
  ...
  let(:catalogue_1) { create :catalogue,
  ...
  let(:catalogue_2) { create :catalogue,
    category_id: category.id,
    sub_category_id: sub_category.id,
    brand_id: brand.id,
    name: "Catalogue-2",
  ...
  it "catalogue has and belongs to many tags" do 
    tag.catalogues << catalogue_1
    tag.catalogues << catalogue_2
    expect(tag.catalogues.count). to eq(2)
  end
end
